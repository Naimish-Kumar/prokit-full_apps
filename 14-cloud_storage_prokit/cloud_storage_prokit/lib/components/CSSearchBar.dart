...
import 'package:cloudstorage_prokit/components/CSFileAndFolderEditingComponents.dart';
...
class CSSearchBar extends SearchDelegate {
  ...
  CSSearchBar({this.hintText, this.listData})
      : super(
  ...
  List<Widget> buildActions(BuildContext context) {
    return [
      query.isEmpty ? 0.width : IconButton(icon: Icon(Icons.clear), color: Colors.black, onPressed: () => query = ""),
    ];
  }
  ...
  Widget buildSuggestions(BuildContext context) {
    final resultList = query.isEmpty
        ...
        : listData!
            .where(
              (element) => element.fileName!.toLowerCase().contains(query.toLowerCase()),
            )
            .toList();
